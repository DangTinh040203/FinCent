...
const attachmentVariants = cva(
  `
    group/attachment bg-card text-card-foreground relative flex w-fit max-w-full
    min-w-0 shrink-0 flex-wrap rounded-xl border transition-colors
    focus-within:ring-ring/50 focus-within:ring-1
...
const attachmentMediaVariants = cva(
  `
    bg-muted text-foreground relative flex aspect-square w-10 shrink-0
...
function AttachmentDescription({
  ...
  return (
    <span
      ...
      className={cn(
        `
          text-muted-foreground mt-0.5 block min-w-0 truncate text-xs
...
function AttachmentGroup({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        `
          scroll-fade-x flex min-w-0 snap-x snap-mandatory scroll-px-1
