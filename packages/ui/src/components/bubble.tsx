...
function BubbleContent({
  ...
  return (
    <Comp
      ...
      className={cn(
        `
          [button,a]:focus-visible:border-ring
          [button,a]:focus-visible:ring-ring/50 [button,a]:transition-colors
          [button,a]:outline-none [button,a]:focus-visible:ring-3
          w-fit max-w-full min-w-0 overflow-hidden rounded-xl border
          border-transparent px-3 py-2 text-sm leading-relaxed wrap-break-word
...
const bubbleReactionsVariants = cva(
  `
    bg-muted ring-card absolute z-10 flex w-fit shrink-0 items-center
    justify-center gap-1 rounded-full px-1.5 py-0.5 text-sm ring-3
    has-[button]:p-0
  `,
  {
    variants: {
